...
- 响应图采用与轴标题同号的正常字重透明标注；I/F 阈值标签固定在左侧同一列，虚线连续；竖虚线与实测曲线交点加圆点，三个载荷响应时间交替分布于曲线两侧
...
- `.xlsx` 与 PNG 导出；菜单栏可选 150 / 300 / 600 PPI，默认 300 PPI
- Windows x86-64 单文件 EXE 自动构建

## V0.8.9 图形标注、默认轴、多速度迟滞与图片分辨率
...
- 导入新数据后，图形默认使用第 1 个原始数据列作为 X 轴，其余原始数据列作为 Y 轴。
- Y 轴已选字段文字使用对应曲线颜色，选择变化后自动同步。
- 详细使用说明见 [V0.8.9 使用说明](docs/V0.8.9_USER_GUIDE.md)。
